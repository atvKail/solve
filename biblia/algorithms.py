    ...
    def binary_search_iterative(array, element): # Бинарный_поиск
        ...
        while start <= end:
            ...
            if element < array[mid]:
                ...
                start = mid + 1
        return -1
    def binary_search_recursive(array, element, start, end):# Бинарный_поиск_Рекурсивный
        if start > end:
            return -1
...
class deleted:
    def deleteDub(sp, sp1):# добавление элементов без дублирования и уничтожения порядка
    ...
    def bubble_sort(a): # Сортировка_пузырьком
        ...
        while unordered:
            ...
            for j in range(n - 1, z, -1):
                if a[j] < a[j - 1]:  # Меняя знак, меняется и порядок. т.е. сортировка по убыванию/по возрастанию
                    a[j], a[j - 1] = a[j - 1], a[j]
                    unordered = True
            z += 1
...
class graphWork():
    ...
    def dijkstra(graph, start):
        ...
        while len(visited) < len(graph):
            current_vertex = min((vertex for vertex in graph if vertex not in visited), key=lambda vertex: distances[vertex])
